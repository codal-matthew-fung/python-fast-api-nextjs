...
export const Table = ({ data }: TableProps) => {
  ...
  return (
    <>
      <div className='mb-20'>
        <p className=''>
          Total Count: <span className='font-bold'>{data.metadata.total_count}</span>
        </p>
        <p className=''>
          Total Pages: <span className='font-bold'>{data.metadata.total_pages}</span>
        </p>
        <p className=''>
          Current Count: <span className='font-bold'>{data.metadata.page_count}</span>
        </p>
        <p className=''>
          Current Page: <span className='font-bold'>{data.metadata.current_page}</span>
        </p>
        <button className='block my-2' onClick={() => router.replace(`${path}`)}>
          Clear Filters
        </button>
        <ul className='inline-flex gap-2'>
          ...
          {data?.metadata?.has_prev_page && (
            <li>
              <button onClick={() => handlePagination({ page: current_page - 1 })}>Previous Page</button>
            </li>
          )}
          <li className='font-bold text-red-500'>{current_page}</li>
          {data?.metadata?.has_next_page && (
            <li>
              <button onClick={() => handlePagination({ page: current_page + 1 })}>Next Page</button>
            </li>
          )}

          {current_page !== data.metadata.total_pages && (
            <li>
              <button onClick={() => handlePagination({ page: data.metadata.total_pages })}>{'>>'}</button>
            </li>
          )}
        </ul>
      </div>
                    ...
                    <th key={key} className={`text-md px-8 py-4 font-semibold text-zinc-600 align-bottom text-left`}>
                      <button
                        className={`text-left text-md font-semibold ${
                          key.includes('isbn') ? 'uppercase' : 'capitalize'
                        }${sort_by_field == key ? ' font-bold' : ''}`}
                        onClick={() => handleSort({ fieldToSort: key, direction: sort_by == 'ASC' ? 'DESC' : 'ASC' })}
                      >
                        {key.replaceAll('_', ' ')} {sort_by_field === key && sort_by}
                      </button>
                    </th>
                  )
                })}
              ...
              <tr key={book.isbn} className={`${idx % 2 != 0 && idx !== 0 ? 'bg-zinc-50' : ''}`}>
                <td className='px-8 py-4'>
                  <ImageWithFallback
                    ...
                    width={100}
                    height={100}
                    customClass='object-contain block h-[100px] w-full max-h-full max-w-full mx-auto object-top-left'
                  />
                </td>
                {Object.keys(book).map((key) => {
                  ...
                  return (
                    <td
                      key={`${book.isbn}-${key}`}
                      className={`px-8 py-4 text-zinc-500 ${
                        key === 'title' ? 'font-bold hover:underline' : 'font-normal'
                      }`}
                    >
                      {key === 'title' ? <Link href={`/book/${book.isbn}`}>{value}</Link> : value}
                    </td>
                  )
                })}
              </tr>
